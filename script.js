  ...
  async function main() {
    ...
    class QuestIO {
      ...
      cacheDOMElements() {
        ...
        this.attachedFileDisplay = document.getElementById(
          "attachedFileDisplay"
        );
      ...
      addEventListeners() {
        ...
        this.topPSlider.addEventListener("input", () => {
          this.topPValue.textContent = parseFloat(
            this.topPSlider.value
          ).toFixed(2);
        });
      ...
      applyTranslations() {
        ...
        document.querySelector(".settings-modal-title").textContent =
          t.settings;
        ...
        document.querySelector('label[for="apiKeyInput"]').textContent =
          t.apiKey;
      ...
      async saveStateToStorage() {
        ...
        try {
          await puter.fs.write(
            "/home/questio_state.json",
            JSON.stringify(appState)
          );
        } catch (error) {
          console.error("Failed to save state to Puter FS:", error);
          puter.ui.showToast({
            content: "Failed to save state.",
            variant: "error",
          });
        }
      }
      ...
      async loadStateFromStorage() {
        try {
          const fileContent = await puter.fs.read("/home/questio_state.json");
          ...
          console.log("No state file found in Puter FS, starting fresh.");
      ...
      renderConversationList() {
        ...
        `;
          ...
          const folderContent =
            folderContainer.querySelector(".folder-content");
      ...
      startFolderNameEdit(folderId) {
        ...
        input.addEventListener("blur", () =>
          this.cancelFolderNameEdit(folderId)
        );
      ...
      handleDrop(e) {
        ...
        const dropIndicatorBefore = document.querySelector(
          ".drop-target-before"
        );
        ...
        if (dropFolderHeader) {
          ...
          if (targetFolderContainer) {
            ...
            if (dropIndicatorBefore)
              ...
              this.state.uncategorizedOrder.splice(
                targetIndex + 1,
                0,
                draggedId
              );
      ...
      sendMessage() {
        ...
        if (isNewConversation) {
          this.state.uncategorizedOrder.unshift(
      ...
      async handleFileUpload() {
        ...
        try {
          ...
          for (const file of files) {
            const stat = await puter.fs.stat(file.path);
      ...
      async getAIResponse(aiBubbleElement, aiMessageId) {
        ...
        try {
          ...
          responseBubble.className =
            "chat-bubble ai-bubble message-text-content";
      ...
      async exportConversation(convId) {
        ...
        const suggestedName = `QuestIO_conversation_${conversation.title.replace(/[^a-z0-9]/gi, "_").toLowerCase()}.json`;
        try {
          const fileHandle = await puter.ui.showSaveFilePicker({
            suggestedName,
          });
          ...
          puter.ui.showToast({ content: "Conversation exported!" });
        } catch (error) {
          console.error("Export failed:", error);
          puter.ui.showToast({ content: "Export failed.", variant: "error" });
        }
      }
      ...
      async exportAllConversations() {
        ...
        const timestamp = new Date()
          ...
          .replace(/:/g, "-")
          .slice(0, 19);
        const suggestedName = `QuestIO_all_conversations_${timestamp}.json`;
        try {
          const fileHandle = await puter.ui.showSaveFilePicker({
            suggestedName,
          });
          ...
          puter.ui.showToast({ content: "All conversations exported!" });
        } catch (error) {
          console.error("Export failed:", error);
          puter.ui.showToast({ content: "Export failed.", variant: "error" });
        }
      ...
      async importConversations() {
        ...
        try {
          ...
          const fileContent = await puter.fs.read(file.path);
          ...
          this.saveStateToStorage();
          this.renderConversationList();
          puter.ui.showToast({ content: t.importSuccess });
        } catch (error) {
          console.error("Import error:", error);
          puter.ui.showToast({ content: t.importError, variant: "error" });
        }
      }
    ...
    // アプリケーションのインスタンスを作成し、非同期で初期化
    const app = new QuestIO();
    await app.init();
  }
